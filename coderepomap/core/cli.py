...
def get_templates_dir() -> Path:
    """Get path to the generic templates directory (coderepomap/templates/)."""
    return Path(__file__).resolve().parent.parent / 'templates'


def get_resources_dir() -> Path:
    """Get path to resources directory (coderepomap/resources/)."""
    return Path(__file__).resolve().parent.parent / 'resources'


def get_lang_templates_dir(lang: str) -> Path:
    """Get path to language-specific templates directory (coderepomap/<lang>/templates/)."""
    return Path(__file__).resolve().parent.parent / lang / 'templates'
...
def cmd_init(args):
    ...
    lang = getattr(args, 'lang', 'csharp') or 'csharp'
    ...
    config_template = _resolve_init_template(lang, preset)

    if config_template is not None and config_template.exists():
        dest = repomap_dir / 'config.yaml'
        if lang == 'lua' and preset != 'unity' and config_template == get_templates_dir() / 'config.generic.yaml':
            # Post-process: rewrite lang: field to lua
            _copy_template_with_lang(config_template, dest, lang='lua')
        else:
            shutil.copy(config_template, dest)
        print(f"Created: .repomap/config.yaml (lang: {lang}, preset: {preset})")
    else:
        # Create default config — but warn loudly that the template was
        # missing. Without this warning users silently get a config with no
        # importance_boost.patterns, producing degraded ranking.
        expected_path = config_template if config_template is not None else "<unresolved>"
        print(
            f"Warning: preset template '{preset}' for lang '{lang}' not found at "
            f"{expected_path}. Falling back to a minimal default config without "
            f"preset-specific boost patterns. The plug-in's default_boost_patterns "
            f"will still apply at generate time.",
            file=sys.stderr,
        )
        ...
        print(f"Created: .repomap/config.{'yaml' if HAS_YAML else 'json'} (template missing, used default)")
    ...
    print(f"\nInitialized coderepomap in {project_root}")
    print("\nNext steps:")
...
def _resolve_init_template(lang: str, preset: str) -> Optional[Path]:
    """Return the config template Path for the given lang+preset combination.

    Routing rules:
      csharp + unity   -> coderepomap/csharp/templates/config.unity.yaml
      csharp + generic -> coderepomap/templates/config.generic.yaml
    ...
    if lang == 'csharp':
        if preset == 'unity':
            return get_lang_templates_dir('csharp') / 'config.unity.yaml'
        else:
...
def _copy_template_with_lang(src: Path, dest: Path, lang: str) -> None:
    """Copy template file to dest, rewriting the `lang:` field to the given value."""
    if HAS_YAML:
        ...
        # Raw text substitution fallback
        text = src.read_text(encoding='utf-8')
        import re
        text = re.sub(r'^(lang\s*:\s*).*$', f'\\g<1>{lang}', text, flags=re.MULTILINE)
...
def cmd_generate(args):
    ...
    try:
        ...
        # Registry error: unregistered language — surface the friendly hint
        print(f"Error: {e}")
        return 1
...
def cmd_status(args):
    ...
    if not repomap_dir.exists():
        ...
        # Still show registered languages even if no project config
        _print_registered_langs()
        return 0
    ...
    # Show registered language parsers
    _print_registered_langs()

    return 0


def _print_registered_langs():
    """Trigger plugin registration and print the registered languages line."""
    # Trigger csharp registration (graceful if extras not installed)
    try:
        import coderepomap.csharp  # noqa: F401
    except ImportError:
        pass

    from .registry import registered_langs
    langs = registered_langs()
    if langs:
        print(f"\nRegistered language parsers: {', '.join(langs)}")
    else:
        print("\nRegistered language parsers: (none)")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        prog='repomap',
        description='Generate layered multi-language code maps',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog='''
Examples:
  repomap init --preset unity          Initialize with Unity preset (C#)
  repomap init --lang lua              Initialize for Lua project
...
'''
    ...
    parser.add_argument(
        '--version', '-V',
        action='version',
        version=f'coderepomap {__version__}'
    )
